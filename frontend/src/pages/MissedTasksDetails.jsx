...
export function MissedTasksDetails() {
    ...
    const fetchTasks = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:3000/api/v1/task/missedtasksdetails",
          {
            headers: {
          ...
          <div className="pt-14">
            {tasks.length === 0 ? (
              ...
              <ul>
                {tasks.map((task) => (
                  <li key={task.taskid}>
                    <div className={`flex justify-between border-4 p-4 hover:border-customGreen items-center ${task.status===0 ? 'bg-red-600' : ''}`}>
                      <div>
                      ...
                      {task.status === 0 && (
                        <div>
                          <Button
                            ...
                            onClick={() =>
                              navigate(
                                `/reasonform?id=${task.taskid}&assignedbyid=${task.taskassignedby}&submissiondate=${task.submissiondate}`
                              )
                            }
                          />
                        </div>
