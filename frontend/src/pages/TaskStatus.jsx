...
export function TaskStatus() {
    ...
    const fetchTasks = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:3000/api/v1/task/totaltasksdetails",
          {
            headers: {
  ...
  loadImageAsBase64("../src/images/Header.png", (base64Image) => {
    ...
    const title = "Assigned Tasks Report";
    ...
    tasks.forEach((task) => {
      ...
      doc.setFont("helvetica", "bold");
      doc.text("Assigned To (Id):", 15, yPosition + 10);
      doc.setFont("helvetica", "normal");
      doc.text(
        `  ${task.taskassignedto}`,
        doc.getTextWidth("Assigned To (Id):") + 17,
        yPosition + 10
      );
      ...
      doc.setTextColor("black");
      doc.setFont("helvetica", "bold");
      doc.text("Assigned To (Name):", 15, yPosition + 10);
      doc.setFont("helvetica", "normal");
      doc.text(
        `  ${task.taskassignedtoname}`,
        doc.getTextWidth("Assigned To (Name):") + 17,
        yPosition + 10
      );
    ...
    const filename = `${user.firstName} ${user.lastName}_Assigned_Tasks_Report.pdf`;
    // Save the PDF
    doc.save(filename);
  });
};
          ...
          <div className="pt-14">
            {tasks.length === 0 ? (
              ...
              <ul>
                {tasks.map((task) => (
                  <li key={task.taskid}>
                    <div className={`flex justify-between border-4 p-4 hover:border-customGreen items-center ${getBackgroundColor(task.status, task.submissiondate)}`}>
                      <div>
                        <strong>Task ID:</strong> {task.taskid}
                        <br />
                        <strong>Status:</strong> {getStatus(task.status,task.submissionDate)}
                        <br />
                        ...
                        <br />
                        <strong>Details provided by you for the task:</strong> {task.message}
                        <hr />
                      </div>
                      {/* <div>
                        <Button
